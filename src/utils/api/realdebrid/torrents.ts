...
export class Torrents extends RealDebridAPI {
  ...
  public async info(torrentId: string): Promise<RealDebridTorrentInfo> {
    const response = await this.makeRequest(`/rest/1.0/torrents/info/${torrentId}`, 'GET', true);
  ...
  public async instantAvailability(torrentHash: string): Promise<{
    ...
    const response = await this.makeRequest(`/rest/1.0/torrents/instantAvailability/${torrentHash}`, 'GET', true);
  ...
  public async activeCount(): Promise<{
    ...
    const response = await this.makeRequest('/rest/1.0/torrents/activeCount', 'GET', true);
  ...
  public async availableHosts(): Promise<{
    ...
    const response = await this.makeRequest('/rest/1.0/torrents/availableHosts', 'GET', true);
  ...
  public async add(host: string): Promise<{
    ...
    const response = await this.makeRequest(`/rest/1.0/torrents/add`, 'PUT', true, JSON.stringify({ host }));
  ...
  public async selectFiles(torrentId: string, files: string): Promise<boolean> {
    ...
    if (!response.ok) {
      if (response.status === 202) {
        ...
        throw new Error('Wrong parameter (invalid file id(s)) / Unknown resource (invalid id)');
      }
      throw new Error(`API request failed with status ${response.status}: ${response.statusText}`);
    }
