export class RealDebridAuth {
  ...
  public async obtainDeviceCode() {
    ...
    if (!response.ok) throw new Error(`Failed to obtain device code: ${response.statusText}`);
    ...
    console.log(`Please go to ${data.verification_url} and enter the code: ${data.user_code}`);

    return data;
  }

  // Step 2: Poll for user authorization and credentials
  public async pollForCredentials(deviceCode: string, interval: number, expiresIn: number) {
  ...
  public async obtainAccessToken(deviceCode: string) {
    ...
    if (!response.ok) throw new Error(`Failed to obtain access token: ${response.statusText}`);
