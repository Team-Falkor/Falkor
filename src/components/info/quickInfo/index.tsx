...
const QuickInfo: FC<QuickInfoProps> = (props) => {
  ...
  const publisher = useMemo(() => involved_companies.find((item) => item.publisher), []);
  const developer = useMemo(() => involved_companies.find((item) => item.developer), []);

  const releaseDate = release_dates?.find((item) => item.platform === 6) ?? release_dates[0];
  ...
  return (
    <section className="mt-3 overflow-hidden text-sm whitespace-normal text-ellipsis text-slate-400">
      <ul className="flex flex-col justify-center gap-2 -mt-1 border-gray-200 divide-gray-200 rounded-lg dark:divide-gray-700 dark:border-gray-700 dark:text-gray-100">
        ...
        <QuickInfoItem
          title={'Rating'}
          data={aggregated_rating ? `${aggregated_rating.toFixed(0)} / 100` : !isReleased ? 'Not Released' : 'N/A'}
        />
        <QuickInfoItem
          title={'Release Date'}
        ...
        <QuickInfoItem
          ...
          data={`${developer?.company.name ?? 'N/A'} / ${publisher?.company.name ?? 'N/A'}` ?? 'N/A'}
        />
      </ul>
    </section>
  );
